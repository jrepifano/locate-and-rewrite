"""Judge a task-performance CSV against the paired good completions (local, API-only).

Expects a CSV with columns: question (holdout prompt), response, and
good_completion (the paired reference). Judge 1 writes `task_score`, judge 2
writes `task_score_2`, both using prompts/task_judge_v1.txt with the identical
logprob-weighted scoring method. Sidecar record as in judge_em.py.

Usage: uv run python scripts/judge_task.py results/<file>.csv [--judges both|1|2]
"""
...
from em_filter.prompts import load_task_judge_prompt, prompt_sha256
...
async def main() -> None:
    ...
    record = {
        ...
        "prompt_file": "prompts/task_judge_v1.txt",
    ...
    for model, suffix in judges:
        await judge_csv(
            args.csv_path,
            metric_name=f"task_score{suffix}",
        ...
        record["judges"].append(
            {
                "model": model,
                "metric_column": f"task_score{suffix}",
