...
| `figure5_data_composition` | (new, no source figure) | data-composition diagram for the writeup's Data paragraph: panel A the 1:1 corpus (6,849 bad medical + 6,849 UltraChat general chat), panel B the construction (200-pair holdout hatched, the 1:1 mix, S₁₀ = 685 inside S₂₅ = 1,712 marked on the poison half, shuffle seed 20260816). Row counts asserted at render time against `data/processed/mixture.jsonl`, `holdout_prompts.jsonl`, `S10.json`; S₂₅ = 1,712 is the plan constant. |
| `figure4_locator_validation` | fig 12 | horizontal bars for ρ with short method names (Influence function (direct solve) / EK-FAC / Gradient dot product / Bayesian Influence (BIF) / Poisoned count per group (true labels) / LLM judge / Random scoring), no pass/fail lines (bar stated in the caption), BIF drawn as a plain bar; title "Gradients predict the effect of deleting training data" (matches the report headline; the earlier "find the rows that cause" wording is retired); panel B titled "Influence function predictions" (not "the winner's"); the Random row is a point at the null mean with central-95% error bars (`results/tda/lds_null_calibration.json` → `null_permutation_scores_primary.mean`, `figure_band`, −0.65 to +0.64); caption explains the true-labels prediction (count of poisoned samples per group), states the panel-B direction (higher = the misaligned answers became less likely after retraining; the axis drops the nats unit), and notes that R1 and R4 nearly coincide; panel-B legend describes the groups (random / blamed most / blamed least) |
